...
void ComputeMV(vector<float> r) 
{
	...
	for (size_t i = 0; i < range_size; i++)
	{
		...
		if (!std::isnan(r[i]) && !std::isinf(r[i]))//取出有效值
		{
			...
			//如果障碍物距离大于等于激光扫描最大距离或者小于激光最小距离，则认为不存在障碍物，赋值scan_distance = 0；否则对障碍物距离进行一个反向，用最远测距距离减去实际检测到的障碍物距离
...
bool IsFrontSafety()
{
	float goal_sector = (int)(0 - (sector_value - sector_scale) + 320) % 360;//(0-(30-10)+360)%360 = 340,goal_sector = 340
	int start_index = goal_sector / angle_resolution;//start_index = 340
	float scan_distance = 0;
	for (int i = 0; i < (sector_value - sector_scale) * 6 / angle_resolution; i++)//for(i=0;i<40;i++)
	{
		...
		for (int i = 0; i < pose.size(); i++)
		{
			while (ros::ok()) {
				...
				if (direction >= -0.5)
				{
					...
					if(fabs(yaw-arc)<0.25)
					{
						pos_target.type_mask = 1 + 2 + /*4 + 8 + 16 + 32 +*/ 64 + 128 + 256 + 512 + 1024 + 2048;
  						pos_target.coordinate_frame = 8;
						pos_target.velocity.x = 0.2;
