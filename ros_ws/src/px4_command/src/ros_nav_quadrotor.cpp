...
PX4RosNav::PX4RosNav(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private): nh_(nh), nh_private_(nh_private) 
{
  initialize();
  cmdloop_timer_ = nh_.createTimer(ros::Duration(0.1), &PX4RosNav::CmdLoopCallback, this); // Define timer for constant loop rate

  cmd_vel_sub_ = nh_private_.subscribe("/cmd_vel", 1, &PX4RosNav::CmdVelCallback, this,ros::TransportHints().tcpNoDelay());
...
{
   OffboardControl_.send_body_velxy_posz_yaw_setpoint(px4_vel_,desire_posz_,yaw_speed);
