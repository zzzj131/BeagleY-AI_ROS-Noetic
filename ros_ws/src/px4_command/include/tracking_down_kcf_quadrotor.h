...
class PX4KcfDownTracking {
 public:
    /**
     *默认构造函数
     */
    PX4KcfDownTracking(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
    /**
     * 析构函数
     */
    ~PX4KcfDownTracking();
 ...
 private:
  ...
  float search_alt_;
  float markers_id_;//需要检测到的目标的ID
  float markers_yaw_;//目标相对飞机的偏航角
  bool detect_state;//是否检测到目标标志位
