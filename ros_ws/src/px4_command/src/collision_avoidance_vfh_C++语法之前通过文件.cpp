...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber gps_sub = nh.subscribe("/mavros/global_position/global",10,gps_cb);
    ...
    ros::Subscriber lidar_sub = nh.subscribe<sensor_msgs::LaserScan>("/lidar2Dscan", 20, scan_cb);

    ros::Subscriber homePos_sub = nh.subscribe<mavros_msgs::HomePosition>("/mavros/home_position/home", 10, home_pos_cb);
    ros::Subscriber head_sub = nh.subscribe<std_msgs::Float64>("/mavros/global_position/compass_hdg", 10, heading_cb);
...
*/		
		  ...
		  while(ros::ok())
		{
		    if( current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(5.0)) && (flag_offboard == 0))
		    {
		        if( set_mode_client.call(offb_set_mode) && offb_set_mode.response.mode_sent)
		        {
		            ROS_INFO("Offboard enabled1");
		            flag_offboard = 1;
		        }
		    ...
		    {
		        if( !current_state.armed && (ros::Time::now() - last_request > ros::Duration(5.0)) && (flag_armed == 0))
		        {
		            if( arming_client.call(arm_cmd) && arm_cmd.response.success)
		            {
		                ROS_INFO("Vehicle armed1");
		                flag_armed = 1;
		            }
		            last_request = ros::Time::now();
		        }
		    }
		    if((flag_armed == 1) && (flag_offboard ==1))
		        break;
		...
		vector<vector<float>> pos_list; //导航坐标点，可自行添加
		vector<float>  v1 {1, 4};
		vector<float>  v2 {2, 5};
		vector<float>  v3 {3, 6};
		pos_list.push_back(v1);
		pos_list.push_back(v2);
		pos_list.push_back(v3);
		
		printf("pos1 x=%f    y=%f\n", pos_list[0][0],pos_list[0][1]);
		printf("pos2 x=%f    y=%f\n", pos_list[1][0],pos_list[1][1]);	
		printf("pos3 x=%f    y=%f\n", pos_list[2][0],pos_list[2][1]);
		...
		//p.pose.position.x = 80;
		//p.pose.position.y = 80;
