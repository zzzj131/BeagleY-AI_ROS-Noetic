...
void altitude_cb(const std_msgs::Float32::ConstPtr& msg)
{
	
	//printf("1111\r\n");
	//sonarMsg.data = *msg;
    sonarMsg = *msg;
    cur_position.pose.position.z = 0.001*sonarMsg.data;
}
...
int main(int argc, char** argv){
  ...
  ros::Subscriber altitude_sub = node.subscribe<std_msgs::Float32>("/ultrasonic", 10, altitude_cb); 
  ...
  while (node.ok()){
    ...
    try{
      transformStamped = tfBuffer.lookupTransform(target_frame_id, source_frame_id, ros::Time(0),ros::Duration(3.0));
